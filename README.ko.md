...
- 2026-06-27 GLM-5.2 상태: `/home/work/.data/huggingface` 아래 FP8 다운로드 완료, Fable/Mythos 스타일 official-agentic SFT mix 19,536 rows 생성 완료. GLM vLLM `0.23.0+cu129` 8xH200 serving/probe는 성공했다. FP8 direct LoRA 학습은 fine-grained FP8 matmul backward 미지원으로 막혔다. BF16 다운로드는 snapshot `f2263102df303b2faa54a6861a29d1770ce846c0`까지 완료했지만, 로컬 Transformers+BitsAndBytes QLoRA도 OOM이다. metadata 점검 결과 BF16 총량 `1403.19GiB`, raw MoE expert weight `1368.00GiB`다. Axolotl FSDP2 8-bit/4-bit MoE LoRA도 train step까지 못 갔다. Megatron-SWIFT는 `glm5_2`/`glm5_1` 템플릿과 데이터 변환까지 통과하지만, BF16/FP8 snapshot 모두 MCore가 BF16 trainable parameter를 만들며 모델 생성 단계에서 GPU당 약 140GiB를 채워 OOM이다. 아직 완료된 GLM-5.2 Fable adapter checkpoint는 없다. 자세한 내용은 [GLM-5.2-FP8 Fable 튜닝 상태](./GLM52_FP8_FABLE_TUNING_STATUS_20260627.ko.md), [GLM-5.2 Fable QLoRA runbook](./GLM52_FABLE_QLORA_RUNBOOK_20260627.ko.md), [GLM-5.2 Fable 출판/다음 학습 계획](./GLM52_FABLE_PUBLICATION_AND_NEXT_TRAINING_PLAN_20260627.ko.md).
- 2026-06-27 GLM-5.2 adapter 출판 경로: Axolotl/bitsandbytes QLoRA는 이 743B MoE를 안정적으로 train step까지 로드하지 못하므로 retired. 유지할 로컬 경로는 **Megatron-SWIFT model-parallel LoRA**이며, 8xH200을 replicated DDP worker가 아니라 TP/PP/EP shard로 쓴다. 다음 smoke를 위해 offload/CPU-init 스위치를 열어뒀다. 관련 스크립트는 `scripts/setup_glm52_swift_megatron_env_20260627.sh`, `scripts/run_glm52_swift_megatron_tp8_lora_20260627.sh`, `scripts/prepare_swift_glm52_agent_jsonl_20260627.py`, `scripts/watch_upload_glm52_fable_lora_20260627.sh`. 공개 adapter target은 [LLM-OS-Models/GLM-5.2-FP8-Agentic-Fable5-Composer2.5-LoRA](https://huggingface.co/LLM-OS-Models/GLM-5.2-FP8-Agentic-Fable5-Composer2.5-LoRA), 카드 원본은 [GLM-5.2-FP8 Agentic Fable5 LoRA README](./model_cards/GLM-5.2-FP8-Agentic-Fable5-Composer2.5-LoRA-README.md).
...
- 문서: [현재 실험 상태](./CURRENT_EXPERIMENT_STATUS_20260624.ko.md), [TB2 vLLM benchmark](./TB2_VLLM_BENCHMARK_20260624.ko.md), [GLM-5.2-FP8 Fable 튜닝 상태](./GLM52_FP8_FABLE_TUNING_STATUS_20260627.ko.md), [GLM-5.2 Fable QLoRA runbook](./GLM52_FABLE_QLORA_RUNBOOK_20260627.ko.md), [GLM-5.2 Fable 출판/다음 학습 계획](./GLM52_FABLE_PUBLICATION_AND_NEXT_TRAINING_PLAN_20260627.ko.md), [GLM-5.2 chaser experiment](./GLM52_CHASER_EXPERIMENT_20260624.ko.md), [multi-model GLM-5.2 chaser plan](./MULTI_MODEL_GLM52_CHASER_PLAN_20260624.ko.md), [DiffusionGemma dLLM eval plan](./DIFFUSIONGEMMA_DLLM_EVAL_PLAN_20260624.ko.md), [DiffusionGemma strength tasks](./DIFFUSIONGEMMA_STRENGTH_TASKS_20260624.ko.md)
- 운영 메모: [data cleanup, git, and GLM training options](./DATA_CLEANUP_GIT_AND_GLM_TRAINING_OPTIONS_20260627.ko.md)
...
- `setup_glm52_vllm_uv_20260627.sh` — Docker 없이 GLM-5.2-FP8 vLLM 0.23 환경 준비
- `run_glm52_fp8_vllm_server_20260627.sh` — 공식 recipe에 가까운 8-way tensor-parallel GLM-5.2-FP8 vLLM server launcher
- `probe_glm52_vllm_server_20260627.py` — 실행 중인 GLM vLLM server용 OpenAI-compatible smoke probe
...
- `setup_glm52_swift_megatron_env_20260627.sh` — GLM-5.2 model-parallel LoRA 시도용 Docker-free Megatron-SWIFT env 준비
- `prepare_swift_glm52_agent_jsonl_20260627.py` — Fable/Mythos agentic mix를 Swift agent/tool-call format으로 변환
- `run_glm52_swift_megatron_tp8_lora_20260627.sh` — TP/PP/EP, offload, batch, target-module env switch를 갖춘 Megatron-SWIFT GLM-5.2 LoRA runner
- `upload_glm52_fable_lora_adapter_20260627.py`, `watch_upload_glm52_fable_lora_20260627.sh` — GLM-5.2 Fable 라인용 adapter-only Hugging Face model-card/checkpoint uploader
...
- `train_glm52_fp8_device_map_lora.py` — Docker 없는 8xH200 GLM-5.2 `device_map=auto` LoRA/QLoRA trainer
- 그 외 데이터 빌더, JSONL 믹서
...
- [GLM-5.2-FP8 Fable 튜닝 상태](./GLM52_FP8_FABLE_TUNING_STATUS_20260627.ko.md)
