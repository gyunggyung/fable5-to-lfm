...
# GLM-5.2-FP8 LoRA runner that shards the frozen FP8 base with device_map=auto.
# This avoids the current torchrun/ZeRO-3 CPU-replication failure mode.
#
# Dry-run:
#   bash fable_distillation/scripts/run_glm52_fp8_device_map_lora_20260627.sh
#
# Smoke after freeing GPUs:
#   RUN_NOW=1 MAX_STEPS=1 MAX_TRAIN_ROWS=2 MAX_SEQ_LENGTH=512 SAVE_STEPS=1 \
#     bash fable_distillation/scripts/run_glm52_fp8_device_map_lora_20260627.sh
#
# Long resumable run:
#   RUN_NOW=1 bash fable_distillation/scripts/run_glm52_fp8_device_map_lora_20260627.sh
...
RUN_NOW="${RUN_NOW:-0}"
RUN_ID="${RUN_ID:-20260627_glm52_fp8_fable_official_agentic_device_map_lora}"
...
OUTPUT_DIR="${OUTPUT_DIR:-$OUTPUT_BASE/GLM-5.2-FP8__Fable-OfficialAgentic-DeviceMap-LoRA-20260627}"
...
CPU_MAX_MEMORY_GIB="${CPU_MAX_MEMORY_GIB:-64}"
...
train_cmd=(
  ...
  TRANSFORMERS_DISABLE_DEEPGEMM_LINEAR="${TRANSFORMERS_DISABLE_DEEPGEMM_LINEAR:-1}"
...
printf 'gpus=%s max_seq=%s steps=%s lr=%s lora_r=%s gpu_max_memory_gib=%s\n' \
  "$TRAIN_GPUS" "$MAX_SEQ_LENGTH" "$MAX_STEPS" "$LEARNING_RATE" "$LORA_RANK" "$GPU_MAX_MEMORY_GIB"
