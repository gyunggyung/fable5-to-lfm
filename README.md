...
- 2026-06-27 GLM-5.2 status: FP8 download completed under `/home/work/.data/huggingface`, Fable/Mythos-style official-agentic SFT mix built with 19,536 rows, and GLM vLLM `0.23.0+cu129` serving/probe succeeded on 8xH200. FP8 direct LoRA training is blocked by the fine-grained FP8 matmul backward path. BF16 download is complete at snapshot `f2263102df303b2faa54a6861a29d1770ce846c0`, but local Transformers+BitsAndBytes QLoRA also OOMs: metadata inspection shows `1403.19GiB` BF16 total, with `1368.00GiB` in raw MoE expert weights. Keep GLM-5.2 local work to FP8 vLLM eval/teacher use unless a GLM-aware FP8/MoE training stack or hosted LoRA is used. See [GLM-5.2-FP8 Fable tuning status](./GLM52_FP8_FABLE_TUNING_STATUS_20260627.ko.md) and [GLM-5.2 Fable QLoRA runbook](./GLM52_FABLE_QLORA_RUNBOOK_20260627.ko.md).
