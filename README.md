...
- 2026-06-27 GLM-5.2 status: FP8 download completed under `/home/work/.data/huggingface`, Fable/Mythos-style official-agentic SFT mix built with 19,536 rows, and GLM vLLM `0.23.0+cu129` serving/probe succeeded on 8xH200. FP8 direct LoRA training is blocked by the fine-grained FP8 matmul backward path, and the older local Transformers+BitsAndBytes BF16 QLoRA path OOMs because the BF16 model is about `1403.19GiB` with `1368.00GiB` in raw MoE expert weights. Current active training path is Axolotl FSDP2 8-bit MoE LoRA with `quantize_moe_experts: true`; the first unchunked 8-bit run hit a CUDA illegal memory access during expert quantization, so `scripts/patch_axolotl_moe_8bit_flatten_20260627.py` now chunks large 3D expert tensors before `bitsandbytes` int8 quantization. Active tmux run: `fable_glm52_axolotl_8bit_chunk_patch2_20260627`, log: `logs/20260627_glm52_axolotl_8bit_moe_lora_chunk_patch2/train.log`, output: `/home/work/.data/harness1/models/GLM-5.2__Fable-OfficialAgentic-Axolotl-8bit-MoE-LoRA-20260627`. See [GLM-5.2-FP8 Fable tuning status](./GLM52_FP8_FABLE_TUNING_STATUS_20260627.ko.md) and [GLM-5.2 Fable QLoRA runbook](./GLM52_FABLE_QLORA_RUNBOOK_20260627.ko.md).
