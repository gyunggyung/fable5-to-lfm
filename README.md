...
> **fable5-to-lfm** — [Fable-5](https://huggingface.co/datasets/Glint-Research/Fable-5-traces) agentic distillation 데이터를 [LiquidAI/LFM2.5-8B-A1B](https://huggingface.co/LiquidAI/LFM2.5-8B-A1B) (8B MoE, ~1B active)에 SFT하여 터미널 에이전트 모델 **Fabliq** 라인을 구축하는 프로젝트.
>
...
이 폴더는 **self-contained** 로 LFM2.5 학습/평가/변환 코드를 모두 포함한다. 외부 의존 파일은 `harness-1/training/`, `harness-1/eval_scripts/`, `Liquid-CLI/scripts/` 에서 복사해왔다.
...
| **Fabliq-8B-Agent-FromBase** | Phase-1B | raw LiquidAI/LFM2.5-8B-A1B | Fable-5 4,047 rows × 3 epoch | 🔄 학습 중 | (예정) |
| **Fabliq-8B-Agent-Large** | Phase-3 | (Phase-2 또는 raw base) | Complete-FABLE.5-traces-2M 50K+ rows × 1 epoch | 📋 계획 | - |
| **Fabliq-8B-Agent-GGUF** | 양자화 | Fabliq-8B-Agent | Q4_K_M/Q5_K_M/Q6_K/Q8_0 | 🔄 변환 중 | (예정) |
...
| `TRAINING_USAGE.md` | 초기 데이터셋 분석 보고서 (한국어, 6종 비교표 + 시나리오 A-E). |
...
| `build_withinus_lfm_sft.py` | `datasets/claude_mythos_distilled_25k/claude_mythos_distilled_25k.jsonl` (25k rows) | `datasets/withinus_lfm_sft_20260623.jsonl` (135 rows) | **Phase-2 WithinUs 빌더.** 카테고리별 균등 샘플링, SHA-256 dedup (25k → 135 unique!), "Drawing from the autonomous..." 템플릿 첫 문장 제거. |
| `build_helio_lfm_sft.py` | `datasets/Fable-5-Distill-Reasoning-462x/Claude-Opus-4.7-4.8-DeepReason-462x-105M.jsonl` (462 rows) | `datasets/helio_lfm_sft_20260623.jsonl` (146 rows) | **Phase-2 Helio 빌더.** Cyrillic 비율 30% 미만 필터 (Russian 우세 행 제거), `<think>` wrapping, line 192 손상 스킵. |
| `build_fable5_2m_to_lfm_sft.py` | `datasets/Complete-FABLE.5-traces-2M/data/train.parquet` (2M events) | `datasets/fable5_2m_lfm_sft_20260623.jsonl` (목표 50K rows) | **Phase-3 빌더.** Parquet row_json에서 Glint 구조화 row (output_type 키 포함) 만 추출, sessionId별 seen_count 필터, build_fable5_to_lfm_sft.convert_row 재사용. |
...
| `scripts/run_fable5_from_base_sft_20260623.sh` | **Phase-1B 러너 (ablation).** raw LiquidAI/LFM2.5-8B-A1B → Fable-5 SFT. ToolBench foundation 효과 분리. LR 1e-6 (base에서 직접이라 좀 더 높게). |
...
| `datasets/Complete-FABLE.5-traces-2M/` | Glint-Research | 2,006,487 | Phase-3 입력 예정. Parquet + jsonl.gz. |
...
| `datasets/fable5_lfm_sft_20260623.meta.json` | - | Phase-1 통계 |
...
# 1. Fable-5 원본 (Glint) → LFM messages JSONL
...
```bash
# 2. WithinUs 25k → 135 rows (aggressive dedup)
python fable_distillation/build_withinus_lfm_sft.py

# 3. Helio 462 → 146 rows (Russian filter)
python fable_distillation/build_helio_lfm_sft.py

# 4. 결합 (Phase-2 reasoning-only)
...
```bash
python fable_distillation/build_fable5_2m_to_lfm_sft.py \
  --max-rows 50000 --seen-count-max 3
```
...
### Agentic tool-use 평가 (vLLM 필요)

```bash
# vLLM 서버 띄우기 (LFM2.5용 런처 사용)
bash Liquid-CLI/scripts/run_lfm25_vllm_replicas_clean.sh &

# 평가 실행
MODEL_PATH=/home/work/.data/harness1/models/.../final_model \
  python fable_distillation/eval_scripts/eval_lfm25_agentic_vllm.py \
    --dataset-path $DATASET_PATH \
    --model-path $MODEL_PATH \
    --served-model-name fabliq \
    --vllm-base-url http://127.0.0.1:8137/v1 \
    --output-jsonl /tmp/eval.jsonl \
    --summary-json /tmp/eval_summary.json
```
...
```bash
# LFM2.5 tokenizer 패치 포함 (llama.cpp 미지원 hash 추가)
...
# HF 업로드 옵션: UPLOAD_REPO_ID=LLM-OS-Models/Fabliq-8B-Agent-GGUF
```
...
- [README.old.md (이전 README)](./README.old.md)
