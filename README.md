...
- 2026-06-27 GLM-5.2 status: FP8 download completed under `/home/work/.data/huggingface`, Fable/Mythos-style official-agentic SFT mix built with 19,536 rows, and GLM vLLM `0.23.0+cu129` serving/probe succeeded on 8xH200. FP8 direct LoRA training is blocked by the fine-grained FP8 matmul backward path, and the older local Transformers+BitsAndBytes BF16 QLoRA path OOMs because the BF16 model is about `1403.19GiB` with `1368.00GiB` in raw MoE expert weights. Axolotl FSDP2 8-bit MoE LoRA with `quantize_moe_experts: true` also did not reach train steps: unchunked loading hit CUDA illegal memory access, chunked int8 loading reached `278/1344` weights (`21%`) then OOM at ~140GiB/GPU. Axolotl 4-bit MoE QLoRA fallback was added, but both raw and 3D-flatten patched runs failed at `40/1344` with bitsandbytes `/src/csrc/ops.cu` invalid configuration. Megatron-SWIFT now reaches GLM/MoE dataset setup with `glm5_2`/`glm5_1` templates, but BF16 and FP8-snapshot runs still OOM during model construction because MCore instantiates BF16 trainable parameters. There is no completed GLM-5.2 Fable adapter checkpoint yet. See [GLM-5.2-FP8 Fable tuning status](./GLM52_FP8_FABLE_TUNING_STATUS_20260627.ko.md), [GLM-5.2 Fable QLoRA runbook](./GLM52_FABLE_QLORA_RUNBOOK_20260627.ko.md), and [GLM-5.2 Fable publication and next-training plan](./GLM52_FABLE_PUBLICATION_AND_NEXT_TRAINING_PLAN_20260627.ko.md).
- 2026-06-27 GLM-5.2 adapter publication route: Axolotl/bitsandbytes QLoRA is retired for this model because it cannot reliably load the 743B MoE for train steps. The maintained local route is **Megatron-SWIFT model-parallel LoRA**, using all 8 H200s as TP/PP/EP shards rather than replicated DDP workers, with offload/CPU-init switches available for the next smoke. Scripts: `scripts/setup_glm52_swift_megatron_env_20260627.sh`, `scripts/run_glm52_swift_megatron_tp8_lora_20260627.sh`, `scripts/prepare_swift_glm52_agent_jsonl_20260627.py`, and `scripts/watch_upload_glm52_fable_lora_20260627.sh`. Public adapter target: [LLM-OS-Models/GLM-5.2-FP8-Agentic-Fable5-Composer2.5-LoRA](https://huggingface.co/LLM-OS-Models/GLM-5.2-FP8-Agentic-Fable5-Composer2.5-LoRA). Model card source: [GLM-5.2-FP8 Agentic Fable5 LoRA README](./model_cards/GLM-5.2-FP8-Agentic-Fable5-Composer2.5-LoRA-README.md).
...
- Docs: [current experiment status](./CURRENT_EXPERIMENT_STATUS_20260624.ko.md), [TB2 vLLM benchmark](./TB2_VLLM_BENCHMARK_20260624.ko.md), [GLM-5.2-FP8 Fable tuning status](./GLM52_FP8_FABLE_TUNING_STATUS_20260627.ko.md), [GLM-5.2 Fable QLoRA runbook](./GLM52_FABLE_QLORA_RUNBOOK_20260627.ko.md), [GLM-5.2 Fable publication and next-training plan](./GLM52_FABLE_PUBLICATION_AND_NEXT_TRAINING_PLAN_20260627.ko.md), [GLM-5.2 chaser experiment](./GLM52_CHASER_EXPERIMENT_20260624.ko.md), [multi-model GLM-5.2 chaser plan](./MULTI_MODEL_GLM52_CHASER_PLAN_20260624.ko.md), [DiffusionGemma dLLM eval plan](./DIFFUSIONGEMMA_DLLM_EVAL_PLAN_20260624.ko.md), [DiffusionGemma strength tasks](./DIFFUSIONGEMMA_STRENGTH_TASKS_20260624.ko.md)
- Ops note: [data cleanup, git, and GLM training options](./DATA_CLEANUP_GIT_AND_GLM_TRAINING_OPTIONS_20260627.ko.md)
...
- `setup_glm52_swift_megatron_env_20260627.sh` — Docker-free Megatron-SWIFT env for GLM-5.2 model-parallel LoRA attempts
- `prepare_swift_glm52_agent_jsonl_20260627.py` — converts the Fable/Mythos agentic mix to Swift agent/tool-call format
- `run_glm52_swift_megatron_tp8_lora_20260627.sh` — Megatron-SWIFT GLM-5.2 LoRA runner with TP/PP/EP, offload, batch, and target-module env switches
- `upload_glm52_fable_lora_adapter_20260627.py`, `watch_upload_glm52_fable_lora_20260627.sh` — adapter-only Hugging Face model-card/checkpoint uploader for the GLM-5.2 Fable line
...
- [GLM-5.2 Fable Publication and Next-Training Plan](./GLM52_FABLE_PUBLICATION_AND_NEXT_TRAINING_PLAN_20260627.ko.md)
