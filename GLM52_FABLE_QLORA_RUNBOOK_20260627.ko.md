...
FP8 cache size는 약 `707G`다. 2026-06-27 13:08 KST 기준 `/home/work/.data` 여유는 약 `1.7T`다.

BF16 원본 `zai-org/GLM-5.2` dry-run 결과는 safetensors `282` shards이고 대부분 shard가 `5.4G`다. 총량은 약 `1.5T`라 다운로드는 가능하지만 매우 빡빡하다. 그래서 학습 output은 full model 저장 없이 LoRA adapter checkpoint만 남긴다.
...
tail -f /home/work/.projects/LLM-OS-Models/Terminal/fable_distillation/logs/20260627_glm52_bf16_download/download.log
df -h /home/work/.data
```
...
다운로드 예상: 현재 저장소가 1.5T급이라 네트워크/Xet cache 속도에 따라 수 시간 단위다. 1차 ETA는 다운로드가 10-20분 이상 진행된 뒤 `download.log`와 디스크 증가량으로 다시 계산한다.
